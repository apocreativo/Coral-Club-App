...
// Limpieza automática de reservas 'pending' vencidas
const cleanupExpired = async () => {
  const now = Date.now();
  const toExpire = data.reservations.filter(r => r.status === "pending" && r.expiresAt && (new Date(r.expiresAt).getTime() <= now));
  if (!toExpire.length) return;
  const tentIds = new Set(toExpire.map(r => r.tentId));
  const tentsUpd = data.tents.map(t => tentIds.has(t.id) ? { ...t, state: "av" } : t);
  const resUpd = data.reservations.map(r => toExpire.some(x => x.id === r.id) ? { ...r, status: "expired" } : r);
  await mergeState({ tents: tentsUpd, reservations: resUpd }, "Auto-expire reservas vencidas");
  if (selectedTent && tentIds.has(selectedTent.id)) {
    setSelectedTent(prev => prev ? { ...prev, state: "av" } : prev);
  }
};
