...
function normalizeState(s) {
  ...
  const payments = {
    ...
    mp: p0.mp || { link: "", alias: "" },
    pagoMovil: p0.pagoMovil || { bank: "", rif: "", phone: "" },
    zelle: p0.zelle || { email: "", name: "" },
  };
  return {
    brand: { name: brand.name || "Coral Club", logoUrl: brand.logoUrl || "/logo.png", logoSize: typeof brand.logoSize === "number" ? brand.logoSize : 42, ...brand },
    background: { publicPath: background.publicPath || "/Mapa.png", ...background },
    layout: s.layout && typeof s.layout === "object" ? s.layout : { count: 20 },
...
export async function kvGet(k) {
  const r = await fetch(`${API_BASE}/get/${encodeURIComponent(k)}`);
  const j = await safeJson(r);
  return decode(j?.result ?? null);
}
export async function kvSet(k, v) {
  const r = await fetch(`${API_BASE}/set/${encodeURIComponent(k)}`, {
    method: "POST",
...
export async function kvIncr(k) {
  const r = await fetch(`${API_BASE}/incr/${encodeURIComponent(k)}`, { method: "POST" });
  const j = await safeJson(r);
