...
export default async function handler(req, res){ const ctx = ctxOr500(res); if(!ctx) return;
  ...
  const upstream = await fetch(`${ctx.url}/set/${encodeURIComponent(key)}`, { method:'POST', headers:{ Authorization:`Bearer ${ctx.token}`,'Content-Type':'application/json' }, body: JSON.stringify({ value: body.value }) });
